...
// show the list of cities searched
var renderCities = function() {
    ...
    if (localStorage.length === 0) {
        ...
        let lastCityKey = "cites" + (localStorage.length - 1);
        ...
        for (let i = 0; i < localStorage.length; i++) {
            ...
            if (city === currentCity) {
                cityEl = `<button type="button" class=" btn-block list-group-item list-group-item-action active">${city}</button></li>`;
            } else {
                cityEl = `<button type"button" class="btn-block list-group-item list-group-item-action">${city}</button></li>`;
            }
            // append city to page
